...
import { getTrendingStocks, getStockData, type TrendingStockReal, type StockData } from '@/lib/trendingStocks';
...
export default function AdminBlogGenerator() {
  ...
  const handleGenerateDailyBlogs = async () => {
    ...
    try {
      ...
      toast.error('Failed to generate daily blogs');
    } finally {
      setIsGenerating(false);
    }
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">Generate professional stock analysis blogs using AI</p>
        </div>
        <div className="flex items-center space-x-2">
          <Badge variant={apiStatus.proxyService ? 'default' : 'secondary'}>
            {apiStatus.proxyService ? '🌐 Proxy Connected' : '❌ Proxy Offline'}
          </Badge>
          <Badge variant={apiStatus.openAI ? 'default' : 'secondary'}>
            {apiStatus.openAI ? '🤖 AI Ready' : '❌ AI Offline'}
          </Badge>
        </div>
      </div>
